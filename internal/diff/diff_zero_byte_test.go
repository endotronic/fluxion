...
func TestZeroByteMoveCopy(t *testing.T) {
	...
	results, err := CompareSnapshots(mapToIter(filesA), mapToIter(filesB), Options{RootA: "/root", RootB: "/root", HashType: "sha1", NoCopies: false, NoMoves: false, ShowUnchanged: false})
	...
	results2, err := CompareSnapshots(mapToIter(filesA2), mapToIter(filesB2), Options{RootA: "/root", RootB: "/root", HashType: "sha1", NoCopies: false, NoMoves: false, ShowUnchanged: false})
	assert.NoError(t, err)
