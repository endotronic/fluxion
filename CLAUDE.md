...
| `knowledge/diff-algo.md` | **before touching `internal/diff` or `internal/app/diff.go` — this is the highest-risk code in the project.** Explains the unified two-snapshot tree, the ten-stage pipeline, `FileTwin` and the presence flags, the synthetic "merkle" directory hashes (a fixed-width digest as of 2026-09-09), the merge-join tree builder and the two-pass oracle it is checked against, the `propagateStatus` rollup precedence, move/copy matching and consumption rules, the hidden-move-source fixed point, and the collapsing logic — plus what the property test asserts and the defects still open. |
| `knowledge/diff-memory.md` | `diff` runs out of RAM, or you are touching how the tree is built or held. Records where the memory goes and what has been reclaimed (**~1 KiB → 178 B/node, 5.75×, as of 2026-09-09** — Phase 0's fixed-width digest plus four more constant-factor cuts, each with a measured before/after), why that still does not reach the 1 GB ceiling, and **Phase 2's streaming engine (2026-09-10), which is off that curve entirely** — `O(depth x budget)`, measured 10.7x lower peak RSS on a real 2.27M-file fleet diff, byte-identical output, but no move/copy detection until Phase 3. Also why the `coverage` command was built first instead. |
