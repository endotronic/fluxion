...
| `knowledge/diff-memory.md` | `diff` runs out of RAM, or you are touching how the tree is built or held. Records where the memory goes and what has been reclaimed (**~1 KiB → 178 B/node, 5.75×, as of 2026-09-09** in the tree engine), and **the streaming engine (Phases 2–3, 2026-09-10), which is off that curve entirely** — `O(depth x budget)` retention plus a fixed sort buffer, move/copy detection as an external sort, **peak RSS flat at ~210 MiB from 1.6M to 6.4M files a side**, byte-identical output. Also the acceptance sweep to repeat after any engine change, and why the `coverage` command was built first. |
