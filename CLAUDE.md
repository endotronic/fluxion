...
| `knowledge/diff-algo.md` | **before touching `internal/diff` or `internal/app/diff.go` — this is the highest-risk code in the project.** Explains the unified two-snapshot tree, the ten-stage pipeline, `FileTwin` and the presence flags, the synthetic "merkle" directory hashes (fixed-width digest as of 2026-09-09, see below), the `propagateStatus` rollup precedence, move/copy matching and consumption rules, the hidden-move-source fixed point, and the collapsing logic — plus what the property test asserts and the defects still open. |
| `knowledge/diff-memory.md` | `diff` runs out of RAM, or you are touching how the tree is built or held. Records where the measured ~1 KiB/node goes (now ~365 B/node — Phase 0's fixed-width digest shipped 2026-09-09, a measured 2.75×), why compaction alone cannot reach a 1 GB ceiling, and the five phases still unbuilt to replace the in-memory tree with a DFS stream plus external sorts — including why the `coverage` command was built first instead. |
